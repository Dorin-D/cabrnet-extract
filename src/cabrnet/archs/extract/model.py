...
class Extract(CaBRNet):
    r"""CaBRNet model implementing the ProtoPNet architecture with intermediate layer extraction available.
    ...
    def __init__(self, extractor: nn.Module, classifier: CaBRNetClassifier, **kwargs):
        ...
        """
        super(Extract, self).__init__(extractor, classifier, **kwargs)
    ...
    def load_state_dict(self, state_dict: dict[str, Any], **kwargs) -> None:  # type:ignore
        ...
        if legacy_state:
            ...
            k_old = "extractor.add_on.conv1.weight"
            state_dict = OrderedDict(('extractor.add_on.convnet.conv1.weight' if k == k_old else k, v) for k, v in state_dict.items())
